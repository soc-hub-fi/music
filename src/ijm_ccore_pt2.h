...
class ijm_ccore_pt2
{
    private:
        ...
        {
            ijm_a_t a = max_ele.real();
            ijm_b_t b = max_ele.imag();

            ijm_cs_sqrt_t cs_sqrt = 0;
            ac_math::ac_sqrt_pwl(max_val,cs_sqrt);
            ijm_cs_reci_t cs_reci = ac_math::ac_reciprocal_pwl<ijm_cs_reci_t>(cs_sqrt);

            ijm_c_arg_t arg_c = a*cs_reci;
            ijm_s_arg_t arg_s = b*cs_reci;
                        ...
                        ijm_diag_max_t max_val,ijm_r_diff_t &r_diff, ijm_norm_max_t &norm_max, ijm_reci2t_t &reci2T)
        {
                ...
                ijm_denom2t_norm_t den2T_norm = CONSTANT_FOUR*norm_max*norm_max;
                ijm_denom2t_diff_t den2T_diff = r_diff*r_diff;
                ijm_denom2t_t den2T = den2T_norm+den2T_diff;
                ijm_sqrt_denom2t_t sqrt_den2T = 0.0;
        ...
        {
            ijm_nom_sin2t_t nom_sin2T = CONSTANT_TWO*norm_max;
            sin2T = nom_sin2T*reci2T;

            //if r_diff < 0, negate sin2T
        ...
        {
            //Cos2T
            ijm_nom_cos2t_t nom_cos2T = 0.0;
            ac_math::ac_abs(r_diff,nom_cos2T);

            ijm_cos2t_t cos2T = nom_cos2T*reci2T;

            //CosT
            ijm_nom_cost_t nom_cosT = CONSTANT_ONE+cos2T;
            ijm_arg_cost_t arg_cosT = nom_cosT/CONSTANT_TWO;
        ...
        {
                ijm_den_sint_t den_sinT = CONSTANT_TWO*cosT;
                ijm_den_sint_reci_t den_sinT_reci = ac_math::ac_reciprocal_pwl<ijm_den_sint_reci_t>(den_sinT);
                        ...
                        ijm_rxx_idx_t row_idx, ijm_rxx_idx_t col_idx, ijm_diag_max_t max_val)
    {
        ...
        ijm_r_cpx_t sinTc = sinT*c;
        ijm_r_cpx_t sinTs = sinT*s;
